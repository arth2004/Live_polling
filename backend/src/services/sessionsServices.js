...
function studentJoin({ name, sessionId }, socket) {
  ...
  const latestPoll = sess.activePollIndex != null
    ? sess.polls[sess.activePollIndex]
    : null
...
function removeStudent({ sessionId, name }) {
  ...
  const targetSocketId = Object
    .entries(sess.students)
    .find(([sockId, studentName]) => studentName === name)?.[0]
...
function cleanupOnDisconnect(socket) {
  for (const [sessionId, sess] of Object.entries(store.sessions)) {
    ...
    if (sess.students[socket.id]) {
      ...
      return { type: 'studentLeave', sessionId, users: Object.values(sess.students) }
    }
  }
  return null
}
